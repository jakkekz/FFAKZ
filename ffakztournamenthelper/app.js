...
function addPlayerToDOM(playerName, playerCountryCode) {
    ...
    playerItem.innerHTML = `
        ...
        <button class="delete-player" onclick="deletePlayer(this)">(Remove)</button>
    `;
...
function deletePlayer(buttonElement) {
    ...
    const playerText = playerItem.textContent.trim();
    ...
    // Filter out the player (match by full text content)
    savedPlayers = savedPlayers.filter(player => 
        `${player.countryCode} - ${player.name}` !== playerText.replace('(Remove)', '').trim()
    );
